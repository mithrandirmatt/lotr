...
This file provides heuristics and guidance to help the agent make good decisions
when instructions are ambiguous or a situation isn't explicitly covered.
...
When a Copilot chat message to the local LLM silently fails, returns nothing, or returns
a truncated/garbled response, work through the call chain in order.
...
VS Code Copilot (BYOM)
  │  port 11436
  ▼
ollama_proxy.py          ← strips think:false tokens, caps max_tokens
  │  port 8787
  ▼
headroom container       ← response compression proxy (ghcr.io/chopratejas/headroom)
  │  port 11434
  ▼
Ollama (host)            ← actual LLM inference
  │  port 3100
  ▼
...
The proxy runs on the **host** as a hidden Python process on port **11436**.
It prints every request to stdout, but since it is launched with `Start-Process -WindowStyle Hidden`
there is no visible window. To troubleshoot:
...
# Then re-run: PowerShell -ExecutionPolicy Bypass -File build/docker/docker.ps1 run
```
...
# Restart MCP container (docker.ps1 run handles this automatically)
wsl -d lotr-docker-service -u root -- docker rm -f lotr-mcp
